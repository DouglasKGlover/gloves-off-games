...
const dateTranslate_smh9jnmMBrXjv9uJrz79SvINQAyvHawE5IBmM1Nyc8A = /* @__PURE__ */ defineNuxtPlugin(() => {
  const dateTranslate = (dateSubmitted) => {
    const parsedDate = new Date(dateSubmitted);
    return {
      short: `${parsedDate.getDate()}/${parsedDate.getMonth() + 1}/${parsedDate.getFullYear() - 2e3}`,
...
const translateLongText_G9fzjXZgDBM_h9Ts_ZLwYcU_z8t97dl4juqqzKyOC7A = /* @__PURE__ */ defineNuxtPlugin(() => {
  const translateLongText = (longText) => {
    const newText = longText.replace(/^### (.*$)/gim, "<h3>$1</h3>").replace(/^## (.*$)/gim, "<h2>$1</h2>").replace(/^# (.*$)/gim, "<h1>$1</h1>").replace(/\*\*(.*)\*\*/gim, "<b>$1</b>").replace(/\*(.*)\*/gim, "<i>$1</i>").replace(/\r\n|\r|\n/gim, "<br>").replace(
      /\[([^\[]+)\](\(([^)]*))\)/gim,
...
const translateRichText__t7CXND3sY5GUrJQaDZEoswKOZ7ryZBeo0fca85SbUQ = /* @__PURE__ */ defineNuxtPlugin(() => {
  const translateRichText = (richText) => {
    const options = {
      renderNode: {
        [INLINES.HYPERLINK]: (node, next) => {
          return `<a href="${node.data.uri}"${!node.data.uri.includes(".") ? "" : ' target="_blank"'}>${next(node.content)}</a>`;
        },
...
export { _export_sfc as _, useRoute as a, asyncDataDefaults as b, createError as c, __nuxt_component_1 as d, entry_default as default, useRouter as e, useRuntimeConfig as f, nuxtLinkDefaults as g, navigateTo as n, resolveRouteObject as r, tryUseNuxtApp as t, useNuxtApp as u };
